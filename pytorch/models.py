...
class Cnn14(nn.Module):
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        embedding = F.dropout(x, p=0.2, training=self.training)  #0.5
...
class Cnn6(nn.Module):
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        x = F.dropout(x, p=0.2, training=self.training)
        x = torch.mean(x, dim=3)
        ########################################## Sparse features are obtained, may be a question, need to delete them to try the
        ########################################## performance of the deep nerual forests
        (x1, _) = torch.max(x, dim=2)
        x2 = torch.mean(x, dim=2)
        x = x1 + x2
        #x = F.dropout(x, p=0.2, training=self.training)            #original p=0.5
        #x = F.relu_(self.fc1(x))
        embedding = F.dropout(x, p=0.2, training=self.training)      #original p=0.5
        clipwise_output = torch.sigmoid(self.fc_audioset(x))
...
class Cnn4(nn.Module):
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        x2 = torch.mean(x, dim=2)
        x = x1 + x2
        #x = F.dropout(x, p=0.2, training=self.training)  # p=0.5#
        #x = F.relu_(self.fc1(x))
        embedding = F.dropout(x, p=0.2, training=self.training)    #p=0.5
        clipwise_output = torch.sigmoid(self.fc_audioset(x))
...
class Cnn3_bian(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
                 fmax, classes_num):

        super(Cnn3_bian, self).__init__()
...
class Cnn2_1024(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
                 fmax, classes_num):

        super(Cnn2_1024, self).__init__()
        ...
        self.fc1 = nn.Linear(128, 128, bias=True)
        self.fc_audioset = nn.Linear(128, classes_num, bias=True)
        #self.fc_audioset = nn.Linear(128, 128, bias=True)
...
class Cnn14_emb512(nn.Module):
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        #x = F.dropout(x, p=0.2, training=self.training)            #0.5
        #x = F.relu_(self.fc1(x))
        embedding = F.dropout(x, p=0.2, training=self.training)     #0.5Cnn14
        clipwise_output = torch.sigmoid(self.fc_audioset(x))
